...
;*****************************************************
;
; Seven segment values for clock
;
;*****************************************************
...
main:
	...
	mov r1, #0x0
	str r1, [r0] ; set time to 00:00
   	str r0, [r0, #4] ; store 0x20001000 at 0x200010004
...
;*
intgpiod: ;Mux
	mov r0,#(GPIOD_GPIOICR & 0xffff) ; reset pin 7
	movt r0,#(GPIOD_GPIOICR >> 16)
	mov r1, #0x80
	str r1, [r0]

	mov  r3,#(0x20001004 & 0xffff) ; Adress pointer
    movt r3,#(0x20001004 >> 16)

    ldr r0, [r3] ; get adress stored at 0x20001004
	...
	ldrb r1, [r0] ; read value from memory
	AND    r1,r1,#0x0F    ; Get 4 least significant bits
	ADR    r2,SJUSEGTAB      ; Table start to r2
	LDRB   r1, [r2,r1]        ; get bitpattern

	mov  r2,#(GPIOB_GPIODATA & 0xffff)
    movt r2,#(GPIOB_GPIODATA >> 16)
    strb r1, [r2] ; output digit to seven segment display
...
increaseMinCounter:
	mov r1, #0x0
	strb r1, [r0, #1] ; set 10 sec back to 0
...
increaseTenMinCounter:
	mov r1, #0x0
	strb r1, [r0, #2] ; set min back to 0
